...
class TestSensorDataValidation:
    ...
    def test_validate_fire_alarm_values(self):
        ...
        # Create base data with valid values for all fields
        base_data = {
            "Temperature[C]": 25.0,
            "Humidity[%]": 50.0,
            "TVOC[ppb]": 100.0,
        ...
        for valid_value in [0, 1, "0", "1"]:
            ...
            assert (
                is_valid is True
            ), f"Fire Alarm value {valid_value} should be valid. Errors: {errors}"
        ...
        for invalid_value in [2, -1, "yes", "no", 0.5]:
            ...
            assert (
                is_valid is False
            ), f"Fire Alarm value {invalid_value} should be invalid"
            assert any("Fire Alarm" in error for error in errors)
...
class TestSensorDataSanitization:
    ...
    def test_sanitize_sensor_data_missing_fields(self):
        ...
        incomplete_data = {"Temperature[C]": 25.5, "Humidity[%]": 45.2}
...
class TestProcessingMetrics:
    ...
    def test_processing_metrics_calculation(self):
        ...
        metrics = ProcessingMetrics.calculate(
            processed_count=95, error_count=5, start_time=start_time
        )
    ...
    def test_processing_metrics_zero_messages(self):
        ...
        metrics = ProcessingMetrics.calculate(
            processed_count=0, error_count=0, start_time=start_time
        )
    ...
    def test_log_processing_metrics_warnings(self, mock_logger):
        ...
        start_time = time.time() - 200.0  # Long duration for low throughput

        metrics = log_processing_metrics(
            50, 50, start_time
        )  # 50% success rate, 0.5 msg/sec throughput
